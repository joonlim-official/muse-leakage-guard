# Iteration-8 review brief — Senior PM
...
YOUR LENS — senior PM. THOROUGH, FULL-SCOPE review: no scope limits, no finding
caps, no timebox. You co-drive the REPORT READABILITY workstream. Focus: can a
non-technical reader understand the generated HTML report? Plain language in
verdicts, findings, and suggestions (no unexplained jargon); scannable
structure (headings, summaries before detail); tri-state semantics consistency
(green = well protected, yellow = protected through approval, red = can leak)
across the report, README, and SKILL.md; residual-gap communication (does the
report say plainly what it does NOT test?); README/CHANGELOG accuracy vs actual
behavior. Generate the report with bin/leakage-report and read it as the
product. Every finding should be actionable as a concrete text or structure
change.
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/03-pm.md
