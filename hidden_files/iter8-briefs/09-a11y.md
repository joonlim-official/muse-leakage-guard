# Iteration-8 review brief — Accessibility specialist
...
You co-drive this workstream with the designer and PM.
...
YOUR LENS — accessibility specialist. THOROUGH, FULL-SCOPE review: no scope
limits, no finding caps, no timebox. Focus: the GENERATED HTML REPORT's
accessibility. Generate it with bin/leakage-report (output to /tmp, never
commit it) and evaluate: screen-reader experience (heading order, landmarks,
table semantics, expandable-card announcements); keyboard navigation
(focusable cards/transcripts, visible focus, skip link); no-JS access to
evidence (does content survive without JavaScript?); color contrast and
non-color status indicators (verdicts must not rely on color alone);
reduced-motion; touch targets on mobile. Test keyboard-only navigation and a
text-mode rendering (e.g. python html2text or lynx if available). Findings
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/09-a11y.md
