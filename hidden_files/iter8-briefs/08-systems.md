# Iteration-8 review brief — Staff systems engineer
...
YOUR LENS — staff systems engineer. THOROUGH, FULL-SCOPE review: no scope
limits, no finding caps, no timebox. Focus: INTERFACE CONTRACTS.
gate-interface.md's machine-readable contract_version — is it actually parsed
and enforced at exactly one point, with one FAIL and no misattributed checks
on mismatch? Exit-code discipline across every stub tool (usage errors vs
verdict codes vs operational failures). Delegate canonicalization and
self-identity guards. Suite structure (A–F) coherence. Messenger stdin-echo
hashing (evidence hashing without leaking content). Version-mismatch fail
behavior. Contracts must be enforced by code, not just documented in prose.
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/08-systems.md
