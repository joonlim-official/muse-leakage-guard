# Iteration-8 review brief — Senior privacy engineer
...
YOUR LENS — senior privacy engineer. THOROUGH, FULL-SCOPE review: no scope
limits, no finding caps, no timebox. Focus: fixture/corpus purity
(synthetic-only invariant — verify no real personal data anywhere in the repo);
SHAPES coverage vs the stub's extractor (is the purity gate a strict subset?);
what the HTML report redacts (currently email-focused — check names, phones,
and other identifiers in evidence text); scrub() behavior; data minimization
in logs and artifacts; residual-gap documentation honesty (KNOWN-GAP markers
vs unexamined silence).
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/02-privacy.md
