# Iteration-8 review brief — Senior security engineer
...
YOUR LENS — senior security engineer. THOROUGH, FULL-SCOPE review: no scope
limits, no finding caps, no timebox. Focus: fail-open/fail-closed behavior in
the stub shims (test/stub-memory-skill/) and bin/leakage-audit; bypass paths
(absolute-path CLI invocation, stdin forms, token confusion, delegate
resolution/canonicalization); exit-code discipline (non-verdict errors must
never reuse 0/1/2); approval-isolation (approval must never unblock block-tier
or mask operational failures); live-fire arming guards. Probe empirically on
/tmp copies where useful — never against real CLIs.
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/01-security.md
