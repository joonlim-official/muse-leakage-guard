# Iteration-8 review brief — Senior SRE
...
YOUR LENS — senior SRE. THOROUGH, FULL-SCOPE review: no scope limits, no
finding caps, no timebox. Focus: OPERATIONAL RELIABILITY of the validation
machinery. The CI workflow (.github/workflows/validate.yml): fail-at-the-end
behavior, timeouts, rc=137 (OOM/sigkill) handling, hermetic delegates
($RUNNER_TEMP vs /usr/local/bin), pinned totals, anchored greps. Slow/hanging
stub fault variants: does the audit fail closed (not hang forever) when a
delegate is slow or flaky? Timeout-budget asymmetry (30s vs 10s paths — is the
asymmetry deliberate and documented?). suite-A build-blockset --check.
Reliability of the harness itself, not just what it asserts.
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/07-sre.md
