# Iteration-8 review brief — Senior AI researcher
...
YOUR LENS — senior AI researcher. THOROUGH, FULL-SCOPE review: no scope
limits, no finding caps, no timebox. Focus: THREAT-MODEL COMPLETENESS.
Compositional/chunked exfiltration (multi-call, field-splitting, cross-session);
shapeless leakage (spelled-out figures, fullwidth digits, nested encoding,
paraphrase, aggregates, oracle-style probing); out-of-band egress
(non-shimmed paths); corpus coverage vs the attack-surface catalog
(references/attack-surface.md); probe-first corpus-expansion methodology
(probe candidates against the real gate; mismatches become KNOWN-GAP entries,
not failing cases); stub/real parity boundaries. Shapes that cannot be
blocked must be cataloged as KNOWN-GAP — never left unexamined.
...
~/workspace/skills/muse-leakage-guard/hidden_files/iter8-reviews/06-airesearcher.md
