...
- A **test double** with the same gate/shim *interface* as the real skill
  ...
  `bin/brief-gate`, `bin/memory-egress-check`, `bin/shims/hatch_gws_cli`,
  `bin/shims/hatch_messenger_cli`.
...
- **Not a detector.** `bin/memory-egress-check` does not detect secrets —
  ...
  (fail closed), but that is a conservative default, not a judgment.
